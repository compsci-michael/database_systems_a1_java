...
public class HMethods {
	...
	// Method to Validate the Input Arguements for dbload
	public boolean input_validation_dbload(String[] args) {
	...
	// Method to Validate the Input Arguements for dbquery
	public boolean input_validation_dbquery(String[] args) {
		boolean is_correct = false;
		boolean override = false;
		// Step 1: Check if number of Arguements is Correct
		if(args.length < 2 || args.length > 2) {
			System.err.println("Error - Not Enough Arguements!");
			System.err.println("Format must be of the following: <search criteria> pagesize\n");
		} else {
			// Step 2: Validate Input
			// Check if the pagesize was a number
			try {
